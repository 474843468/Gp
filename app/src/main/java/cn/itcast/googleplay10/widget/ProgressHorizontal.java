...
import cn.itcast.googleplay10.utils.StringUtils;
...
public class ProgressHorizontal extends View {
	...
	public void setProgressTextColor(ColorStateList color) {
		...
		mProgressTextColor = mProgressTextColorStateList.getColorForState(
				getDrawableState(), mProgressTextColor);
	}
	...
	private void notifyProgressChange(float smoothProgress, float targetProgress) {
		if (null != mOnProgressChangeListener) {
			mOnProgressChangeListener.onProgressChange(smoothProgress,
					targetProgress);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode == MeasureSpec.EXACTLY) {
			...
			width = mDrbBackground == null ? 0 : mDrbBackground
					.getIntrinsicWidth();//getIntrinsicWidth取得Drawable的固有的宽度
		...
		if (heightMode == MeasureSpec.EXACTLY) {
			...
			height = mDrbBackground == null ? 0 : mDrbBackground
					.getIntrinsicHeight();
		...
		mRawProgressBounds.set(getPaddingLeft(), getPaddingTop(), width
				- getPaddingRight(), height - getPaddingBottom());
		setMeasuredDimension(width, height);
		/**
		 * onMeasure()，视图大小的将在这里最终确定，也就是说measure只是对onMeasure的一个包装，子类可以覆写onMeasure()方法实现自己
		 * 的计算视图大小的方式，并通过setMeasuredDimension(width, height)保存计算结果。
		 */
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		mSmoothProgress = mStartProgress + factor
		...
		if (null != mDrbProgress) {
			if (mDrbProgress instanceof NinePatchDrawable
					|| (mDrbProgress instanceof DrawableContainer && ((DrawableContainer) mDrbProgress)
							.getCurrent() instanceof NinePatchDrawable)) {
				if (mSmoothProgress == 0) {
					...
					mDrbProgress
							.setBounds(
									...
									(int) (mRawProgressBounds.left + (mRawProgressBounds
											.width() - mProgressDrbMinWidth)
											* mSmoothProgress)
											+ mProgressDrbMinWidth,
		...
		if (mProgressTextVisible) {
			...
			canvas.drawText(text, getWidth() >> 1, ((getHeight()
	...
	protected void drawableStateChanged() {
		...
		if (mProgressTextColorStateList != null) {
			mProgressTextColor = mProgressTextColorStateList.getColorForState(
					drawableState, mProgressTextColor);
		}
		invalidate();
	}
