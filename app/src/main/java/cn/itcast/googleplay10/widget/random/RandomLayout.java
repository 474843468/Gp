package cn.itcast.googleplay10.widget.random;
...
public class RandomLayout extends ViewGroup {
	...
	public void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		// 确定自身的宽高
		int thisW = r - l - this.getPaddingLeft() - this.getPaddingRight();//右边减去左边
		int thisH = b - t - this.getPaddingTop() - this.getPaddingBottom();//底部减去顶部
		// 自身内容区域的右边界和下边界
			...
			if (!mFixedViews.contains(child)) {//mFixedViews用于存放已经确定好位置的View，存到了就没必要再次存放
				...
				while (availAreaCount > 0) {
					...
					if (mAreaDensity[row][col] < areaCapacity) {
						...
						int rightEdge = contentRight - childW;	// 自身内容区域的右边界-子View测量之后的宽
